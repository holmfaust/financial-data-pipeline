...
def create_volume_chart(data, symbol):
    ...
    fig = go.Figure(
        data=[go.Bar(x=df["timestamp"], y=df["volume"], name="Volume", marker_color="lightblue")]
    )
...
def main():
    ...
    # Header
    st.markdown(
        '<div class="main-header">📈 Real-time Financial Data Pipeline</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        selected_symbol = st.selectbox("Select Stock Symbol", default_symbols, index=0)

        # Time range
        time_range = st.selectbox(
            "Time Range", ["1 Hour", "4 Hours", "1 Day", "3 Days", "7 Days"], index=2
        )

        time_map = {"1 Hour": 1 / 24, "4 Hours": 4 / 24, "1 Day": 1, "3 Days": 3, "7 Days": 7}
    ...
    try:
        ...
        if agg_metrics:
            ...
            with col1:
                # SMA chart
                sma_fig = create_line_chart(
                    agg_metrics, selected_symbol, "calculated_sma_20", "Simple Moving Average (20)"
                )
                if sma_fig:
            ...
            with col2:
                # Volatility chart
                vol_fig = create_line_chart(
                    agg_metrics, selected_symbol, "volatility", "Price Volatility"
                )
                if vol_fig:
