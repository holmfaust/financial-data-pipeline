...
class DatabaseManager:
    ...
    def get_aggregated_metrics(
        self, symbol: str, window: str = "5min", limit: int = 100
    ) -> List[Dict]:
        """Get aggregated metrics"""

        table_map = {"1min": "stock_metrics_1min", "5min": "view_stock_metrics_indicators"}
    ...
    def get_anomalies(self, symbol: Optional[str] = None, hours: int = 24) -> List[Dict]:
    ...
    def insert_pipeline_run(
        ...
        with self.get_connection() as conn:
            ...
            cur.execute(
                query,
                (pipeline_name, run_id, status, records_processed, records_failed, error_message),
            )
        ...
        with self.get_connection() as conn:
            ...
            if result["latest_timestamp"]:
                metrics["data_freshness_minutes"] = (
                    datetime.now() - result["latest_timestamp"]
                ).total_seconds() / 60
