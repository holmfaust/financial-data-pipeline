...
class FinancialStreamProcessor:
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        spark = (
            SparkSession.builder.appName("FinancialDataStreamProcessor")
            .config(
                "spark.jars.packages",
    ...
    def process_stock_stream(self):
        ...
        # Convert timestamp
        parsed_stream = parsed_stream.withColumn("timestamp", to_timestamp(col("timestamp")))
        ...
        query1 = self._write_to_postgres(one_min_agg, "stock_metrics_1min", "1min_checkpoint")

        query2 = self._write_to_postgres(five_min_agg, "stock_metrics_5min", "5min_checkpoint")

        query3 = self._write_to_postgres(anomalies, "price_anomalies", "anomalies_checkpoint")

        # Also write raw data
        query4 = self._write_raw_to_postgres(parsed_stream, "stock_prices_raw", "raw_checkpoint")
    ...
    def _calculate_1min_metrics(self, stream):
        ...
        agg_stream = stream.groupBy(window(col("timestamp"), "1 minute"), col("symbol")).agg(
    ...
    def _calculate_5min_metrics(self, stream):
        ...
        agg_stream = stream.groupBy(window(col("timestamp"), "5 minutes"), col("symbol")).agg(
            spark_min("low_price").alias("low_price"),
            spark_max("high_price").alias("high_price"),
            avg("close_price").alias("avg_price"),
    ...
    def _detect_anomalies(self, stream):
        ...
        baseline = stream.groupBy(
            ...
            col("symbol"),
        ).agg(avg("close_price").alias("mean_price"), stddev("close_price").alias("std_price"))
        ...
        current_window = stream.groupBy(window(col("timestamp"), "1 minute"), col("symbol")).agg(
        ...
        result = anomalies.select(
            ...
            ((col("max_price") - col("min_price")) / col("current_price") * 100).alias(
                "deviation_pct"
            ),
            when((col("max_price") - col("min_price")) / col("current_price") > 0.1, "high")
            .otherwise("medium")
            .alias("severity"),
        )
            ...
            try:
                if batch_df.count() > 0:
                    batch_df.write.mode("append").jdbc(
                        self.db_url, table_name, properties=self.db_properties
                    )
                    logger.info(
                        f"Batch {batch_id} written to {table_name}: {batch_df.count()} records"
                    )
